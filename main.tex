...
\Inout edr: EdR
    ...
    \\rtaValida(s) \yLuego
    \\\norm{s} = \norm{edr.solucion}
    \\0 \leq posiblesAccesos \leq cuantosEstudiantes(edr.aula)}
    \aseguraLargo{
    \existe {n}{\Z}{0 \leq n \leq posiblesAccesos} \implica
...
\Tipo{Examen}
    ...
    \requiereLargo{
    perteneceAlumno(e, edr.aula) \yLuego\\
    hayPreguntaSinResponder(e.examen[\norm{e.examen}-1]) \yLuego\\
    edr0 = edr
    }
    \aseguraLargo{
        ...
        (\exists k: \Z)(0 \leq k < \norm{e.examen[0]} \yLuego\\
        mismosEjerciciosMenos(k, res[-1], e.examen[-1]) \land\\
        \lnot respondio(k, e.examen[-1]) \land
        respondio(k, res[-1]))\yLuego\\
        % vv aca me fijo que el resto de pasos esten igual
        mismosPasos(e.examen, res) \yLuego\\
        % vv igualo el examen en edr con el examen q devuelvo
        edr.aula[e.asiento.f][e.asiento.c].examen = res.examen\yLuego\\
        % vv Todo el resto sigue igual
        todosEstudiantesIgualesMenos(e.asiento, edr.aula, edr0.aula)\yLuego\\
        edr0.solucion = edr.solucion \yLuego\\
        edr0.entregas = edr.entregas
    }
\end{proc}
...
(\norm{p} > 0)\land(\exists i:\Z)(0 \leq i < \norm{x[0]} \yLuego p[\norm{p}][i]="")
}
