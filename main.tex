...
\Tipo{}
    ...
    \aseguraLargo{
        \existe{j}{\Z}{(suExamen(c1, edr)[j] = "" \land suExamen(c2, edr)[j] \neq "") \implicaLuego
        \\ suExamen(c1, edr) = setAt(suExamen(c1,edr_0, j, suExamen(c2, edr_0)[j]))} \yLuego
        \\ \paraTodo{c3}{Coordenada}{(coordenadaValida(c3, edr.aula) \land c1 \neq c3) \implicaLuego
        \\ edr.aula[c3.f][c3.c] = edr_0.aula[c3.f][c3.c]} \yLuego
...
\predLargo{perteneceAlumno}{c:Coordenada, a:Aula}{
    (coordenadaValida(c, edr.aula) \yLuego a[c.f][c.c] \neq \langle \rangle)
}
...
\Inout edr: EdR
    ...
    \aseguraLargo{
    \existe {n}{\Z}{0 \leq n \leq posiblesAccesos \land 0 \leq n \leq cuantosEstudiantes(edr.aula)} \yLuego
    \\\existe {seCopian}{seq<Coordenada>}{\norm{seCopian} = n} \yLuego \\\paraTodo{c}{Coordenada}{c \in seCopian \implicaLuego \\(coordenadaValida(c,edr.aula) \land edr.aula[c.f][c.c] \neq <> \land\\
    edr.aula[c.f][c.c].examen = s)} \yLuego
    \\edr.solucion = edr0.solucion \yLuego
    \\edr.entregas = edr0.entregas \yLuego
    \\todosEstudiantesIgualesMenos(seCopian,edr.aula, edr0.aula)}
\end{proc}

\predLargo{todosEstudiantesIgualesMenos}{coords: seq<Coordenada>, a0, a1: Aula}{
        \norm{a0} = \norm{a1}\land\\
        \paraTodo{c}{Coordenada}{(coordenadaValida(c, a0) \land c \notin coords) \implicaLuego a1[c.f][c.c] = a0[c.f][c.c]}
}
...
\Tipo{Examen}
    ...
    \aseguraLargo{
        ...
        todosEstudiantesIgualesMenos(<a.coord>, edr.aula, edr0.aula)\yLuego\\ %está bien la sintaxis de pasarle una secuencia solo con a.coord? cómo sería?
