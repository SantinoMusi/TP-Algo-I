...
\Tipo{EdR}
    ...
    \aseguraLargo{
    \norm{res.aula} = dimensionAula \land
    \\ aulaCuadrada(res.aula) \land
    \\ cuantosEstudiantes(res.aula) = cantEstudiantes \land
    \\ examenesInicializados(res.aula, s) \land
    \\ noHayAlumnosJuntos(res.aula) \land
    \\ todosAlumnosConCoordenada(res.aula) \land
    \\ res.solucion = s \land 
    \\ res.entregas = \conj{}}
\end{proc}
...
Chequea que la cantidad de estudiantes presentes sea menor o igual a los que entran en el aula manteniendo la separación de al menos 1 asiento entre cada uno \\
...
\predLargo{perteneceAlumno}{e: Alumno, a:Aula}{
    e.examen \neq \langle \rangle \land coordenadaValida(e.coord, a) \land \\ suExamen(e.cord, a) = e.examen
}
...
\Tipo{\seq{Examen}}
    ...
    \requiereLargo{
    perteneceAlumno(a, edr.aula)\land\\
    examenInicializado(a.examen, edr.solucion)\land\\
    \norm{e} > 0 \land\\
    edr_0 = edr
    }
    \aseguraLargo{
        ...
        alumnoTieneEsteExamen(res[\norm{res}-1], a.coord, edr.aula)\land\\
        % vv Todo el resto sigue igual
        todosEstudiantesIgualesMenos(\langle a.coord \rangle, edr.aula, edr_0.aula)\land\\ 
        edr_0.solucion = edr.solucion \land\\
        edr_0.entregas = edr.entregas
    }
\end{proc}

\predLargo{alumnoTieneEsteExamen}{e: Examen, c: Coordenada, a: Aula}{
    a[c.f][c.c] = \struct{coord:c, examen:e}
}
...
\Inout edr: EdR
    ...
    \requiereLargo{
        perteneceAlumno(a, edr.Aula) \land \\ edr = edr_0
    } 
    \aseguraLargo{
        % CAMBIO EL a.examen, ANTES REVISABA EL ULTIMO PASO
        edr.entregas = edr_0.entregas \cup \conj{a} \land
        \\ suExamen(a.coord, edr.aula) = \langle \rangle \land
        \\ todosEstudiantesIgualesMenos(\langle a.coord \rangle,edr,edr_0) \land
...
\Tipo{\seq{Alumno}}
    ...
    \asegura{
        \paraTodo {a1}{Alumno}{a1 \in edr.entregas \implicaLuego (a1 \in res \leftrightarrow esSospechoso(a1)) \land \\
