...
\\ $(\sum_{i=0}^{\norm{entregas}-1}IfThenElse(a1.examen = entregas[i].examen,1,0)) \cdot \frac{100}{\norm{entregas}}$
\\
...
{
\In edr:EdR,
\In sospechosos:\seq{Alumno}
}{
\Tipo{\seq{\tupla{Alumno, Nota}}}
}
    \requiere{True}
    \aseguraLargo{
        \paraTodo{i}{\Z}{(0 \leq i < \norm{edr.entregas} \yLuego \lnot (edr.entregas[i] \in sospechosos)) \implica
        \\ (\tupla{edr.entregas[i], nota(edr.entregas[i], edr.solucion)} \in res)}
    }
\end{proc}
